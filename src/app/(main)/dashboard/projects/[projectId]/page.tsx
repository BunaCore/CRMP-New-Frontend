...
function ProjectWorkspacesContent() {
  ...
  return (
    <div className="anim-in fade-in slide-in-from-bottom-5 mx-auto flex w-full max-w-7xl flex-1 flex-col gap-8 p-4 pt-0 duration-700 md:p-8">
      ...
      <div className="flex flex-col gap-6">
        <div className="flex items-center gap-4">
          <Button
            variant="ghost"
            size="icon"
            className="hover:bg-primary/5 h-10 w-10 rounded-full"
          ...
          <div className="flex flex-col">
            ...
            <p className="text-muted-foreground mt-1 flex items-center gap-2 text-sm font-medium tracking-wider uppercase">
        ...
        <div className="bg-card flex items-center justify-between rounded-2xl border p-6 shadow-sm">
          ...
          <Button className="shadow-primary/20 gap-2 shadow-lg" onClick={handleCreateWorkspace}>
      ...
      {/* Workspaces Table */}
      <Card className="border-muted-foreground/10 overflow-hidden shadow-xl">
        <Table>
                ...
                >
                  <TableCell className="py-5">
                    <div className="flex items-center gap-3">
                      <div className="bg-primary/5 text-primary group-hover:bg-primary flex h-10 w-10 items-center justify-center rounded-xl transition-all duration-300 group-hover:text-white">
                        <FileText className="h-5 w-5" />
                      </div>
                      <span className="inline-block text-base font-semibold transition-transform group-hover:translate-x-1">
                        {ws.title}
                      </span>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <div className="border-border bg-secondary flex h-7 w-7 items-center justify-center rounded-full border">
                        <User className="text-muted-foreground h-4 w-4" />
                      </div>
                      <span className="text-sm font-medium">{ws.manager}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {ws.tasks?.map((task, i) => (
                        <Badge
                          key={`${ws.id}-task-${i}`}
                          variant="outline"
                          className="bg-background/50 text-[10px] font-bold tracking-tighter uppercase"
                        >
                          {task}
                        </Badge>
                      ))}
                  ...
                  <TableCell className="pr-8 text-right">
                    <Button
                      variant="ghost"
                      size="icon"
                      className="group-hover:bg-primary h-8 w-8 rounded-full transition-colors group-hover:text-white"
                    >
