    ...
    const simulateIncomingMessage = () => {
      ...
      if (Math.random() < 0.3) {
        ...
        const onlineMembers = roomMembers.filter(
          (id) => teamMembers.find((m) => m.id === id)?.isOnline
        )
    ...
    const markAsRead = () => {
      setGroupMessages((prev) => ({
        ...
        [selectedRoom.id]: (prev[selectedRoom.id] || []).map((msg) => ({
          ...msg,
          isRead: true,
          readBy: msg.readBy.includes(CURRENT_USER_ID)
            ? msg.readBy
            : [...msg.readBy, CURRENT_USER_ID],
        })),
      }))
  ...
  const handleViewRoomInfo = () => {
    if (selectedRoom) {
      if (selectedRoom.type === "direct") {
        ...
        const partner = teamMembers.find((m) => m.id === partnerId)
        if (partner) {
          alert(`Direct Message with ${partner.name}\nRole: ${partner.role}\nDepartment: ${partner.department}\nEmail: ${partner.email}`)
        }
      } else {
        ...
        {selectedRoom ? (
          ...
          <div className="flex flex-1 flex-col items-center justify-center bg-card">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Select a chat room or start a direct message
            </p>
          </div>
        )}
      </div>
