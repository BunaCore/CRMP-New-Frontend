...
export function ChatRoomsList({
  ...
  return (
    <div className="flex h-full flex-col border-r bg-background">
      ...
      <div className="border-t p-3">
        <p className="mb-2 text-xs font-medium text-muted-foreground uppercase">Active Now</p>
          ...
          {members
            ...
            .map((member) => (
              <button
                ...
                onClick={() => {
                  const existingDM = rooms.find(
                    (r) => r.type === "direct" && r.memberIds.includes(member.id) && r.memberIds.includes(currentUserId)
                  )
                  if (existingDM) {
